...
def gbfs(start,goal,graph):
    ...
    visited=set()
    while PQ:
        heuristic_cost,node,path=heapq.heappop(PQ)
        if(node==goal):
            return heuristic_cost,path
        if node not in visited:
            visited.add(node)
            for neighbors,cost in graph.get(node,[]):
...
def astar(start,goal,graph):
    ...
    while PQ:
        ...
        for neighbors,g_score in graph.get(node,[]):
            ...
            if neighbors not in visited or new_g<visited[neighbors]:
